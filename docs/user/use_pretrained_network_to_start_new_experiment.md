...
Before attempting either of these, you'll need a saved experiment snapshot. [This page](save_load_resume_exp.md)
...
can be found [here](reuse_garage_policy.md). Next, we setup a new experiment and
pass the policy as the `source` argument of the `BC` constructor:
...
Please refer to [this page](algo_bc.md) for more information on garage's implementation
of Behavioral Cloning. If your expert policy wasn't trained with garage, you can
wrap it in garage's `Policy` API (`garage.torch.policies.Policy`) before passing
it to `BC`.
